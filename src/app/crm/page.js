...
export default function CRMDashboard() {
  ...
  // Функция перемещения карточек по клику на стрелочки (теперь работает везде)
  const moveTask = (taskId, sourceColId, direction) => {
  ...
  const handleSave = () => {
    ...
    if (editingTask.isNew) {
      ...
      newData.columns["col-1"].taskIds.unshift(newTask.id);
    } else {
    ...
    <div className="flex flex-col md:flex-row h-screen bg-[#0a0a0a] text-gray-300 font-sans selection:bg-purple-500/30 relative">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-x-auto flex flex-col">
        <header className="mb-6 md:mb-8 shrink-0 flex justify-between items-center">
          <div><h2 className="text-xl md:text-2xl font-bold text-white uppercase tracking-wider">Контроль экспортных сделок</h2></div>
        ...
        {/* Убрали DragDropContext, оставили только чистую верстку */}
          ...
          {data.columnOrder.map((columnId, colIndex) => {
            ...
            return (
              <div key={column.id} className="bg-[#111] border border-gray-800 rounded-lg w-80 flex flex-col shrink-0 max-h-full">
                ...
                <div className="flex-1 p-3 space-y-4 overflow-y-auto min-h-[150px]">
                  {tasks.map((task) => {
                    const progress = calculateProgress(task.checklist);
                    return (
                      <div key={task.id} className="bg-[#0a0a0a] border border-gray-700 p-4 rounded group relative">
                        ...
                        {/* КНОПКИ ПЕРЕМЕЩЕНИЯ (ТЕПЕРЬ ВИДНЫ ВЕЗДЕ) */}
                        <div className="flex justify-between mt-3 border-t border-gray-800 pt-3">
                          <button onClick={() => moveTask(task.id, column.id, 'left')} disabled={colIndex === 0} className={`text-[10px] px-3 py-1 rounded font-bold uppercase ${colIndex === 0 ? 'text-gray-700 bg-gray-900 cursor-not-allowed' : 'text-purple-400 bg-purple-900/30 hover:bg-purple-900/50'}`}>&larr; Назад</button>
                          <button onClick={() => moveTask(task.id, column.id, 'right')} disabled={colIndex === data.columnOrder.length - 1} className={`text-[10px] px-3 py-1 rounded font-bold uppercase ${colIndex === data.columnOrder.length - 1 ? 'text-gray-700 bg-gray-900 cursor-not-allowed' : 'text-purple-400 bg-purple-900/30 hover:bg-purple-900/50'}`}>Вперед &rarr;</button>
                        </div>
                      </div>
                    );
                  })}
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#111] border border-gray-800 rounded-xl w-full max-w-md p-6 shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-mono text-blue-500 mb-1 uppercase">Номер контейнера (для трекинга)</label>
                <input type="text" value={editingTask.containerId || ""} onChange={(e) => setEditingTask({...editingTask, containerId: e.target.value.toUpperCase()})} className="w-full bg-[#0a0a0a] border border-blue-900/50 focus:border-blue-500 rounded p-2 text-white outline-none font-mono tracking-widest" placeholder="MSKU1234567"/>
              </div>
            </div>

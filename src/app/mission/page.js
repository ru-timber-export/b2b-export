...
export default function MissionPage() {
  const { mission, updateMission, missionStats, hydrated } = useDeal();
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <section className="bg-gradient-to-br from-blue-900/50 to-slate-800/50 border border-cyan-500/30 rounded-2xl p-6 backdrop-blur">
          <div className="flex justify-between items-baseline mb-3">
            <h2 className="font-bold text-lg text-cyan-300">📊 Общий прогресс миссии</h2>
            <button
              onClick={() => setEditMode(!editMode)}
              className="text-xs bg-slate-700 hover:bg-slate-600 px-3 py-1 rounded transition-all active:scale-95"
            >
              {editMode ? "✓ Готово" : "✏ Изменить"}
            </button>
          </div>

          <div className="text-3xl md:text-5xl font-black font-mono mb-2 text-white">
        ...
        {editMode && (
          <section className="bg-slate-800/80 border border-slate-700 rounded-xl p-5 space-y-4 backdrop-blur">
            ...
            <div className="border-t border-slate-700 pt-4">
              <h4 className="text-sm font-bold text-slate-300 mb-3">🎯 Цели (можно подкрутить):</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <GoalCard
            ...
            current={Math.min(mission.currentCapital * (mission.goal_ship / missionStats.totalGoal), mission.goal_ship)}
          ...
          <GoalCard
            ...
            current={Math.min(mission.currentCapital * (mission.goal_house / missionStats.totalGoal), mission.goal_house)}
          ...
          <GoalCard
            ...
            current={Math.min(mission.currentCapital * (mission.goal_wedding / missionStats.totalGoal), mission.goal_wedding)}
          ...
          <GoalCard
            ...
            current={Math.min(mission.currentCapital * (mission.goal_reserve / missionStats.totalGoal), mission.goal_reserve)}
        ...
        <section className="bg-gradient-to-br from-orange-900/30 to-amber-900/20 border border-orange-500/30 rounded-2xl p-6 backdrop-blur">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div className="bg-slate-900/60 rounded-lg p-4 border border-emerald-500/20">
              <div className="text-xs text-slate-400 uppercase tracking-wider">Прибыль в месяц</div>
              <div className="text-2xl font-black text-emerald-300 mt-1">{fmtShort(missionStats.profitPerMonthRub)} ₽</div>
              <div className="text-xs text-slate-500 mt-1">{mission.containersPerMonth} конт. × ${fmt(mission.avgProfitPerContainer_usd)}</div>
            </div>
            <div className="bg-slate-900/60 rounded-lg p-4 border border-cyan-500/20">
              <div className="text-xs text-slate-400 uppercase tracking-wider">Прибыль в год</div>
          ...
          <div className="bg-slate-900/40 rounded-lg p-4 text-sm text-slate-300 border-l-4 border-orange-500">
            ...
            }</strong>. Это {(missionStats.monthsNeeded - Math.ceil(missionStats.containersNeeded / 3))} месяцев раньше.
          </div>
        </section>
